...
const Cart = () => {
  ...
  const handleQuantity = (id, qty, stock, type, size) => {
    if (type === 1) {
      const newQty = qty + 1;
      if (newQty > stock) return;
      dispatch(updateItem(id, newQty, size));
    } else {
      const newQty = qty - 1;
      if (newQty < 1) return;
      dispatch(updateItem(id, newQty, size));
              ...
              cart.map(data => (
                <div className="cart-item" key={data.id}>
                  ...
                  <div className="item-quantity">
                    <button onClick={() => handleQuantity(data.id, data.qty, data.stock, 1, data.size)}>+</button>
                    <input type="number" value={data.qty} readOnly />
                    <button onClick={() => handleQuantity(data.id, data.qty, data.stock, 0, data.size)}>-</button>
                  </div>
